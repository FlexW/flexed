...
public:
  ...
  void get_cmd_bar_input(const Glib::ustring& prompt, C* instance) {
    ...
    cmd_bar.get_buffer()->insert_with_tag(cmd_bar.get_buffer()->begin(),
                                          prompt + CMD_BAR_PROMPT_SEPERATOR, no_editable_tag);
...
private:
   ...
   * @param old_tv The TextView that created this view. Needed for taking over buffer_container.
   ...
   */
  paned* create_paned(Gtk::ScrolledWindow* left_side, Gtk::Orientation orientation);
   ...
   */
  void remove_last_paned(paned* active_paned, Gtk::ScrolledWindow* active_sw,
             Gsv::View* active_tv);
   ...
   * @param active_tv Active TextView.
   */
  void remove_other_paned(paned* active_paned, Gtk::ScrolledWindow* active_sw,
              Gsv::View* active_tv);
   ...
   * @param old_container Container from TextView, that opend this TextView.
   */
  ...
  /**
   * Opens a text file.
   * Creates a buffer for the file, takes care that not one file is opend twice.
   ...
   * Every time the focus changed on a editor_text_view, this handler gets called.
  ...
  static __attribute__((always_inline)) void class_method_stub(instance_ptr instance, Glib::ustring data) {
