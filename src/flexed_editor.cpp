...
editor::editor()
  ...
  //g_keyboard_map
  //  ->set_key_binding<editor, &editor::set_divider_from_active_paned_middle>("CxCc", this);
  g_keyboard_map
...
/*
bool editor::on_key_press_event(GdkEventKey* key_event) {
/*if((key_event->keyval == 'g') && (key_event->state & GDK_CONTROL_MASK)) {
    // Ctrl - g pressed.
    global_abort();
    return false;
    }
  keyboard.on_key_pressed(key_event);
  return false;
}
*/
...
paned* editor::create_paned(Gtk::ScrolledWindow* left_side, Gtk::Orientation orientation) {
...
void editor::remove_paned() {
  ...
  if ((Gtk::Box*)active_paned->get_parent() == &main_box) {
    remove_last_paned(active_paned, (Gtk::ScrolledWindow*)active_sw, (Gsv::View*)active_tv);
    reset_active_text_view(active_tv);
    return;
  }
  remove_other_paned(active_paned, (Gtk::ScrolledWindow*)active_sw, (Gsv::View*)active_tv);
  reset_active_text_view(active_tv);
}

void editor::remove_last_paned(paned* active_paned, Gtk::ScrolledWindow* active_sw,
                    Gsv::View* active_tv) {
...
void editor::remove_other_paned(paned* active_paned, Gtk::ScrolledWindow* active_sw,
                     Gsv::View* active_tv) {
  active_paned->remove(*active_sw);
  remove_text_view_from_map(active_tv);
  delete active_tv;
...
void editor::remove_text_view_from_map(Gsv::View* view) {
  ...
  //free(iter->first); // Free text_buffer_container.
}

void editor::add_text_view_to_map(Gsv::View* view,
                       orderd_container<Glib::RefPtr <text_buffer> >& old_container) {
  text_buffer_container* c = new text_buffer_container(g_text_buffer_container, view);
  text_view_map.insert(std::pair<Gsv::View*, text_buffer_container*>(view, c));
  c->take_over_text_buffer_container_list(old_container);

  view->signal_focus_in_event()
    .connect(sigc::mem_fun(*this, &editor::on_text_view_focus_changed), false);
}
...
Glib::RefPtr<text_buffer> editor::create_text_buffer(std::string& buffer_name) {
  auto new_buffer = Glib::RefPtr<text_buffer>(new text_buffer(this));
  new_buffer->set_name(buffer_name);
  g_text_buffer_container->add(static_cast< Glib::RefPtr<text_buffer> >(new_buffer));
...
void editor::switch_paned_up() {
  ...
  if (get_paned_orientation(ppaned) == Gtk::ORIENTATION_VERTICAL && is_paned_child2(active_text_view)) {
  ...
  } while ((get_paned_orientation(parent_paned) == Gtk::ORIENTATION_VERTICAL && is_paned_child1(ppaned))
           || get_paned_orientation(parent_paned) == Gtk::ORIENTATION_HORIZONTAL);
  //if (get_paned_orientation(parent_paned) == Gtk::ORIENTATION_VERTICAL && is_paned_child2(ppaned)) {
...
void editor::switch_paned_down() {
  ...
  if (get_paned_orientation(ppaned) == Gtk::ORIENTATION_VERTICAL && is_paned_child1(active_text_view)) {
  ...
  } while ((get_paned_orientation(parent_paned) == Gtk::ORIENTATION_VERTICAL && is_paned_child2(ppaned))
           || get_paned_orientation(parent_paned) == Gtk::ORIENTATION_HORIZONTAL);
  // if (get_paned_orientation(parent_paned) == Gtk::ORIENTATION_VERTICAL && is_paned_child1(ppaned)) {
...
void editor::switch_paned_left() {
  ...
  if (get_paned_orientation(ppaned) == Gtk::ORIENTATION_HORIZONTAL && is_paned_child2(active_text_view)) {
      set_focus(*find_text_view_in_paned_child1(ppaned));
      return;
  }
  auto parent_paned = ppaned;
  ...
  } while ((get_paned_orientation(parent_paned) == Gtk::ORIENTATION_HORIZONTAL && is_paned_child1(ppaned))
           || get_paned_orientation(parent_paned) == Gtk::ORIENTATION_VERTICAL);
  //if (get_paned_orientation(parent_paned) == Gtk::ORIENTATION_HORIZONTAL && is_paned_child2(ppaned)) {
    set_focus(*find_text_view_in_paned_child1(parent_paned));
    //}
}
...
void editor::switch_paned_right() {
    ...
    if (get_paned_orientation(ppaned) == Gtk::ORIENTATION_HORIZONTAL && is_paned_child1(active_text_view)) {
      g_print("set focus\n");
      set_focus(*find_text_view_in_paned_child2(ppaned));
      return;
    }
    ...
    } while ((get_paned_orientation(parent_paned) == Gtk::ORIENTATION_HORIZONTAL && is_paned_child2(ppaned))
             || get_paned_orientation(parent_paned) == Gtk::ORIENTATION_VERTICAL);
    //if (get_paned_orientation(parent_paned) == Gtk::ORIENTATION_HORIZONTAL && is_paned_child1(ppaned)) {
        set_focus(*find_text_view_in_paned_child2(parent_paned));
        //}
}
...
void editor::on_buffer_changed() {
    ...
    keyboard.set_keyboard_map(get_active_text_view_buffer()->get_keyboard_map());
}
...
void editor::load_mode() {
  get_cmd_bar_input<editor, &editor::load_mode_callback>("Load mode", this);
}
...
void editor::call_mode_function_callback(Glib::ustring name) {
  ...
  //fmode_loader.call_function((std::string&)name);
}
